...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
    ...
    def live_update(self, string=None):
        ...
        if self.fade: return
        if string:
            string = re.sub(r'\W+', '', unidecode(string.lower()))  # remove caracteres especiais e acentos
            self._deiconify()
            i = 1
            for key in self.widgets.keys():
                s = re.sub(r'\W+', '', unidecode(self.widgets[key].cget("text").lower()))  # remove caracteres especiais e acentos
                similar = s.startswith(string)
                if similar:
